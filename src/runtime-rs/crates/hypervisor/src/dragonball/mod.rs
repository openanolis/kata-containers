...
use crate::{Backend, DeviceType, Hypervisor, NetworkConfig, VcpuThreadIds,MemoryConfig};
...
impl Hypervisor for Dragonball {
    ...
    async fn resize_memory(&self, old_mem_mb: u32, new_mem_mb: u32) -> Result<(u32, MemoryConfig)> {
        let mut inner = self.inner.write().await;
        inner.resize_memory(old_mem_mb, new_mem_mb)
    }
}
