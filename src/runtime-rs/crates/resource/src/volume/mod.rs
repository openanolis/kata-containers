...
impl VolumeResource {
    ...
    pub async fn handler_volumes(
        ...
        for m in oci_mounts {
            ...
            info!(sl!(), "get mount_info {:?} for mount {:?}", &mount_info, &m);

            let volume: Arc<dyn Volume> =
                // if let Ok(mount_info) = mount::get_volume_mount_info(&m.source) {
                    if let Ok(mount_info) = mount_info {
                    // direct volume
                    if secure_volume::is_secure_volume(&mount_info) {
                        Arc::new(
                            secure_volume::SecureVolume::new(m, &mount_info, cid)
