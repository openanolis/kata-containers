...
const KATAOVERLAYDEVTYPE: &str = "overlayfs";
        ...
        let (rootfs_storage, rootfs_guest_path) = match extra_options.fs_version.as_str() {
            NYDUS_ROOTFS_V5 => {
                ...
                options.push(
                    "workdir=".to_string()
                        + &do_get_guest_path(
                            format!("{}/{}", SNAPSHOTDIR, "work").as_str(),
                            cid,
                            false,
                            false,
                        ),
                ...
                Ok((
                    Storage {
                        driver: KATAOVERLAYDEVTYPE.to_string(),
            ...
            // TODO
            NYDUS_ROOTFS_V6 => {
                // As some kernel patch is not support in the upstream community, we will delivery this feature later.
                // see this issue (https://github.com/kata-containers/kata-containers/issues/5143)
                let errstr: &str = "new_nydus_rootfs: nydus v6 unavailable currently";
                error!(sl!(), "{}", errstr);
                Err(anyhow!(errstr))
            }
            _ => {
