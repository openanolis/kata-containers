...
impl VfioDeviceMgr {
    ...
    pub fn insert_device(
    ...
    ) -> std::result::Result<(), VfioDeviceError> {
        ...
        if ctx.is_hotplug {
            ...
            let res = self.add_device(ctx, &config, device_index);
            if let Err(e) = res {
                return Err(e);
            }
        }
                    ...
                    UpcallClientResponse::DevMgr(response) => {
                        if let DevMgrResponse::Other(resp) = response {
                            if let Err(e) = result_sender.send(Some(resp.result)) {
                                info!("send upcall result failed, due to {:?}!", e);
                            }
                        }
                    }
    ...
    pub fn start_devices(&mut self, vm_as: &GuestAddressSpaceImpl) -> Result<()> {
        if self.vfio_container.is_some() {
            slog::info!(self.logger, "[CHAOTETS] start device");
    ...
    fn create_device(
        ...
        ctx: &mut DeviceOpContext,
        idx: usize,
    ) -> std::result::Result<Arc<dyn DeviceIo>, VfioDeviceError> {
        let sysfs_path = Self::build_sysfs_path(cfg)?;
        slog::info!(self.logger, "[CHAOTEST] start attach");
...
impl VfioDeviceMgr {
    ...
    ) -> std::result::Result<&mut Arc<PciSystemManager>, VfioDeviceError> {
